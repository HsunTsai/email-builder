<template>
  <BaseColumnsContainer :props="restProps" :style="style">
    <template #column-0>
      <EditorChildrenIds
        :children-ids="columns?.[0]?.childrenIds"
        @change="handleUpdateColumns(0, $event)"
      />
    </template>
    <template #column-1>
      <EditorChildrenIds
        :children-ids="columns?.[1]?.childrenIds"
        @change="handleUpdateColumns(1, $event)"
      />
    </template>
    <template #column-2>
      <EditorChildrenIds
        :children-ids="columns?.[2]?.childrenIds"
        @change="handleUpdateColumns(2, $event)"
      />
    </template>
