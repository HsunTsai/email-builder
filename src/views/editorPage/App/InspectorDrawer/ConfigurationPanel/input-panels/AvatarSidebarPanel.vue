<template>
  <BaseSidebarPanel title="Avatar block">
    ...
    <RadioGroupInput
      ...
      @update:model-value="
        handleUpdateData({ ...data, props: { ...data.props, shape: $event } })
      "
    />
    <div class="space-y-2">
...
const imageUrl = computed(
  () => props.data.props?.imageUrl ?? AvatarPropsDefaults.imageUrl,
);
const alt = computed(() => props.data.props?.alt ?? AvatarPropsDefaults.alt);
const shape = computed(
  () => props.data.props?.shape ?? AvatarPropsDefaults.shape,
);
