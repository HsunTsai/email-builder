<template>
  <div class="min-h-screen bg-gray-50">
    ...
    <div class="flex h-screen">
      <!-- Sidebar -->
      <div class="w-80 bg-white shadow-sm border-r border-gray-200 overflow-y-auto">
        <div class="p-6">
          <!-- Tools Section -->
          <div class="mb-8">
            ...
            <div class="grid grid-cols-2 gap-3">
              ...
              >
                ...
                <div class="text-sm font-medium text-gray-700 group-hover:text-blue-600">
                  {{ tool.name }}
                </div>
              </button>
            </div>
          ...
          <div>
            ...
            <div class="space-y-2">
              <div 
                ...
                :class="{ 'ring-2 ring-blue-500 bg-blue-50': selectedElement === element }"
                @click="selectElement(element)"
              >
                <div class="flex items-center">
                  <span class="text-lg mr-3">{{ getElementIcon(element.type) }}</span>
              ...
              <div v-if="elements.length === 0" class="text-center py-8 text-gray-500">
              ...
              >
                ...
                >
                  ...
                  <div v-if="element.type === 'text'">
                    <h3 v-if="element.title" class="text-lg font-semibold text-gray-900 mb-2">
                      {{ element.title }}
                    </h3>
                  ...
                  <!-- Button Element -->
                  <div v-else-if="element.type === 'button'" class="text-center">
                  ...
                  <div v-else-if="element.type === 'image'" class="text-center">
                    <div class="bg-gray-200 rounded-lg p-8 border-2 border-dashed border-gray-300">
                      <svg class="w-16 h-16 mx-auto text-gray-400 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                      </svg>
                      <p class="text-gray-500">{{ element.title || '圖片佔位符' }}</p>
                    </div>
                  </div>
                ...
                <div v-if="elements.length === 0" class="text-center py-20">
                  ...
                  <h3 class="text-lg font-medium text-gray-900 mb-2">開始建立您的郵件</h3>
                  <p class="text-gray-500 mb-6">從左側工具列選擇元件來開始設計</p>
                  <div class="text-sm text-gray-400 space-y-1">
                    <p>當前路由基礎路徑: <code class="bg-gray-100 px-2 py-1 rounded">{{ $router.options.history.base }}</code></p>
                    <p>當前路由: <code class="bg-gray-100 px-2 py-1 rounded">{{ $route.path }}</code></p>
                  </div>
                </div>
              </div>
            </div>
  ...
  methods: {
    selectTool(tool) {
      const newElement = {
        type: tool.type,
        title: tool.name,
        content: tool.type === 'text' ? '這是一段示例文字，請修改為您需要的內容。' : '',
